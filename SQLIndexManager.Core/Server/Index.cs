      ...
      if (IndexType == IndexType.HEAP && FixType == IndexOp.CREATE_COLUMNSTORE_INDEX) {
        ...
        switch (FixType) {
          ...
          case IndexOp.REBUILD_COLUMNSTORE_ARCHIVE:
            ...
            sql = $"ALTER INDEX {fullIndexName} REBUILD PARTITION = {partition}{Environment.NewLine}    " +
                    $"WITH (DATA_COMPRESSION = {(FixType == IndexOp.REBUILD ? DataCompression : compression)}, MAXDOP = {Settings.Settings.Options.MaxDop});";
            break;
        ...
        switch (FixType) {
          ...
          case IndexOp.CREATE_INDEX:
            ...
            else if (Settings.Settings.Options.DataCompression != DataCompression.DEFAULT && FixType != IndexOp.REBUILD)
              compression = Settings.Settings.Options.DataCompression;
            else
              compression = DataCompression;

            string onlineRebuild = "OFF";
            if (FixType == IndexOp.REBUILD_ONLINE || (Settings.Settings.Options.Online && IsAllowOnlineRebuild)) {
              if (Settings.Settings.Options.WaitAtLowPriority && Settings.Settings.ServerInfo.MajorVersion >= ServerVersion.Sql2014)
                onlineRebuild = "ON (" +
                                  $"WAIT_AT_LOW_PRIORITY (MAX_DURATION = {Settings.Settings.Options.MaxDuration} MINUTES, " +
                                  $"ABORT_AFTER_WAIT = {Settings.Settings.Options.AbortAfterWait}))";
              else
                onlineRebuild = "ON";
            }
            ...
            string nr = Settings.Settings.Options.NoRecompute == NoRecompute.DEFAULT 
                            ? ((IsNoRecompute ?? false) ? "ON" : "OFF")
                            : Settings.Settings.Options.NoRecompute.ToString();

            sql = sqlHeader +
                    "WITH (" +
                    (IndexType == IndexType.HEAP
                      ? ""
                      : $"SORT_IN_TEMPDB = {Settings.Settings.Options.SortInTempDb.OnOff()}, ") +
                    (IsPartitioned || IndexType == IndexType.HEAP
                      ? ""
                      : $"PAD_INDEX = {Settings.Settings.Options.PadIndex.OnOff()}, ") +
                    (IsPartitioned || Settings.Settings.Options.FillFactor == 0
                      ? ""
                      : $"FILLFACTOR = {Settings.Settings.Options.FillFactor}, ") +
                    ...
                    (Settings.Settings.ServerInfo.MajorVersion <= ServerVersion.Sql2012 && IsPartitioned // 'ONLINE' is not a recognized ALTER INDEX REBUILD PARTITION option
                      ? ""
                      : $"ONLINE = {onlineRebuild}, ") +
                    $"MAXDOP = {Settings.Settings.Options.MaxDop});";
            break;

          case IndexOp.REORGANIZE:
            sql = $"ALTER INDEX {fullIndexName} REORGANIZE PARTITION = {partition}{Environment.NewLine}    " +
                    $"WITH (LOB_COMPACTION = {Settings.Settings.Options.LobCompaction.OnOff()});";
            break;
          ...
          case IndexOp.UPDATE_STATISTICS_FULL:
            string nr2 =    (Settings.Settings.Options.NoRecompute == NoRecompute.DEFAULT && (IsNoRecompute ?? false)) 
                         || Settings.Settings.Options.NoRecompute == NoRecompute.ON
                            ? ", NORECOMPUTE"
                            : "";

            sql = $"UPDATE STATISTICS {objectName} {indexName}{Environment.NewLine}    " + (
                FixType == IndexOp.UPDATE_STATISTICS_SAMPLE
                    ? $"WITH SAMPLE {Settings.Settings.Options.SampleStatsPercent} PERCENT{nr2};"
